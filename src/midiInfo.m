...
if (isempty(tracklist))
  tracklist = 1:length(midi.track);    %空缺代表所有音轨 [] for all
end

current_tempo = 500000;  % default tempo 拍子/速度
...
%  如果是文件，就创建文件并将midi信息写入
% (不是N*8矩阵)，而是mid.track.message
fid = -1;
